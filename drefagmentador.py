...
class DiscoDuro:
    ...
    def mostrar_particiones(self):
        for particion in self.particiones:
            print(particion)
            particion.mostrar_estado_grafico()
...
class Particion:
    ...
    def defragmentar(self):
        ...
        # Actualizar la tabla de asignación en los archivos
        for i, archivo_info in enumerate(self.archivos):
            archivo, inicio, fin = archivo_info
            self.archivos[i] = (archivo, nuevo_inicio, nuevo_inicio + (fin - inicio))
        
        self.tabla_asignacion = nueva_tabla_asignacion

    def mostrar_estado_grafico(self):
        plt.figure(figsize=(10, 3))
        plt.bar(range(len(self.tabla_asignacion)), self.tabla_asignacion, color=['green' if c == 1 else 'red' for c in self.tabla_asignacion])
        plt.title(f"Estado de la partición {self.nombre}")
...
if __name__ == "__main__":
    ...
    # Mostrar particiones y archivos antes de la defragmentación
    print("Estado del disco antes de la defragmentación:")
    ...
    time.sleep(2)  # Simular un tiempo de espera para la defragmentación en vivo
    print("\nEstado del disco después de la defragmentación:")
    disco.mostrar_particiones()
    for particion in disco.particiones:
        print(f"Archivos en {particion.nombre}:")
        for archivo, inicio, fin in particion.archivos:
